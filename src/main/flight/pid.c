...
#define PID_PROCESS_DENOM_DEFAULT       1
 ...
 {
	 pidConfig.pid_process_denom = PID_PROCESS_DENOM_DEFAULT;
...
pidProfile_t pidProfiles[PID_PROFILE_COUNT];

 void pidProfiles_Init(void)
 {
	 for (int i = 0; i < PID_PROFILE_COUNT; i++) {
		 pidProfiles[i].pid[PID_ROLL].P = 45;
		 pidProfiles[i].pid[PID_ROLL].I = 80;
		 pidProfiles[i].pid[PID_ROLL].D = 40;
		 pidProfiles[i].pid[PID_ROLL].F = 120;

		 pidProfiles[i].pid[PID_PITCH].P = 47;
		 pidProfiles[i].pid[PID_PITCH].I = 84;
		 pidProfiles[i].pid[PID_PITCH].D = 46;
		 pidProfiles[i].pid[PID_PITCH].F = 125;

		 pidProfiles[i].pid[PID_YAW].P = 45;
		 pidProfiles[i].pid[PID_YAW].I = 80;
		 pidProfiles[i].pid[PID_YAW].D = 0;
		 pidProfiles[i].pid[PID_YAW].F = 120;

		 pidProfiles[i].pid[PID_LEVEL].P = 50;
		 pidProfiles[i].pid[PID_LEVEL].I = 50;
		 pidProfiles[i].pid[PID_LEVEL].D = 75;
		 pidProfiles[i].pid[PID_LEVEL].F = 0;

		 pidProfiles[i].pid[PID_MAG].P = 40;
		 pidProfiles[i].pid[PID_MAG].I = 0;
		 pidProfiles[i].pid[PID_MAG].D = 0;
		 pidProfiles[i].pid[PID_MAG].F = 0;

		 pidProfiles[i].pidSumLimit = PIDSUM_LIMIT;
		 pidProfiles[i].pidSumLimitYaw = PIDSUM_LIMIT_YAW;
		 pidProfiles[i].yaw_lowpass_hz = 100;
		 pidProfiles[i].dterm_notch_hz = 0;
		 pidProfiles[i].dterm_notch_cutoff = 0;
		 pidProfiles[i].itermWindupPointPercent = 85;
		 pidProfiles[i].pidAtMinThrottle = PID_STABILISATION_ON;
		 pidProfiles[i].levelAngleLimit = 55;
		 pidProfiles[i].feedforward_transition = 0;
		 pidProfiles[i].yawRateAccelLimit = 0;
		 pidProfiles[i].rateAccelLimit = 0;
		 pidProfiles[i].itermThrottleThreshold = 250;
		 pidProfiles[i].itermAcceleratorGain = 3500;
		 pidProfiles[i].crash_time = 500;          // ms
		 pidProfiles[i].crash_delay = 0;           // ms
		 pidProfiles[i].crash_recovery_angle = 10; // degrees
		 pidProfiles[i].crash_recovery_rate = 100; // degrees/second
		 pidProfiles[i].crash_dthreshold = 50;     // degrees/second/second
		 pidProfiles[i].crash_gthreshold = 400;    // degrees/second
		 pidProfiles[i].crash_setpoint_threshold = 350; // degrees/second
		 pidProfiles[i].crash_recovery = PID_CRASH_RECOVERY_OFF; // off by default
		 pidProfiles[i].horizon_tilt_effect = 75;
		 pidProfiles[i].horizon_tilt_expert_mode = false;
		 pidProfiles[i].crash_limit_yaw = 200;
		 pidProfiles[i].itermLimit = 400;
		 pidProfiles[i].throttle_boost = 5;
		 pidProfiles[i].throttle_boost_cutoff = 15;
		 pidProfiles[i].iterm_rotation = false;
		 pidProfiles[i].iterm_relax = ITERM_RELAX_RP;
		 pidProfiles[i].iterm_relax_cutoff = ITERM_RELAX_CUTOFF_DEFAULT;
		 pidProfiles[i].iterm_relax_type = ITERM_RELAX_SETPOINT;
		 pidProfiles[i].acro_trainer_angle_limit = 20;
		 pidProfiles[i].acro_trainer_lookahead_ms = 50;
		 pidProfiles[i].acro_trainer_debug_axis = FD_ROLL;
		 pidProfiles[i].acro_trainer_gain = 75;
		 pidProfiles[i].abs_control_gain = 0;
		 pidProfiles[i].abs_control_limit = 90;
		 pidProfiles[i].abs_control_error_limit = 20;
		 pidProfiles[i].abs_control_cutoff = 11;
		 pidProfiles[i].antiGravityMode = ANTI_GRAVITY_SMOOTH;
		 pidProfiles[i].dterm_lpf1_static_hz = DTERM_LPF1_DYN_MIN_HZ_DEFAULT;
             // NOTE: dynamic lpf is enabled by default so this setting is actually
             // overridden and the static lowpass 1 is disabled. We can't set this
             // value to 0 otherwise Configurator versions 10.4 and earlier will also
             // reset the lowpass filter type to PT1 overriding the desired BIQUAD setting.
		 pidProfiles[i].dterm_lpf2_static_hz = DTERM_LPF2_HZ_DEFAULT;   // second Dterm LPF ON by default
		 pidProfiles[i].dterm_lpf1_type = FILTER_PT1;
		 pidProfiles[i].dterm_lpf2_type = FILTER_PT1;
		 pidProfiles[i].dterm_lpf1_dyn_min_hz = DTERM_LPF1_DYN_MIN_HZ_DEFAULT;
		 pidProfiles[i].dterm_lpf1_dyn_max_hz = DTERM_LPF1_DYN_MAX_HZ_DEFAULT;
		 pidProfiles[i].launchControlMode = LAUNCH_CONTROL_MODE_NORMAL;
		 pidProfiles[i].launchControlThrottlePercent = 20;
		 pidProfiles[i].launchControlAngleLimit = 0;
		 pidProfiles[i].launchControlGain = 40;
		 pidProfiles[i].launchControlAllowTriggerReset = true;
		 pidProfiles[i].use_integrated_yaw = false;
		 pidProfiles[i].integrated_yaw_relax = 200;
		 pidProfiles[i].thrustLinearization = 0;

		 pidProfiles[i].d_min[X] = 30;
		 pidProfiles[i].d_min[Y] = 34;
		 pidProfiles[i].d_min[Z] = 0;

		 pidProfiles[i].d_min_gain = 37;
		 pidProfiles[i].d_min_advance = 20;
		 pidProfiles[i].motor_output_limit = 100;
		 pidProfiles[i].auto_profile_cell_count = AUTO_PROFILE_CELL_COUNT_STAY;
		 pidProfiles[i].transient_throttle_limit = 0;
		 pidProfiles[i].profileName[MAX_PROFILE_NAME_LENGTH + 1] = 0;
		 pidProfiles[i].dyn_idle_min_rpm = 0;
		 pidProfiles[i].dyn_idle_p_gain = 50;
		 pidProfiles[i].dyn_idle_i_gain = 50;
		 pidProfiles[i].dyn_idle_d_gain = 50;
		 pidProfiles[i].dyn_idle_max_increase = 150;
		 pidProfiles[i].feedforward_averaging = FEEDFORWARD_AVERAGING_OFF;
		 pidProfiles[i].feedforward_max_rate_limit = 90;
		 pidProfiles[i].feedforward_smooth_factor = 25;
		 pidProfiles[i].feedforward_jitter_factor = 7;
		 pidProfiles[i].feedforward_boost = 15;
		 pidProfiles[i].dterm_lpf1_dyn_expo = 5;
		 pidProfiles[i].level_race_mode = false;
		 pidProfiles[i].vbat_sag_compensation = 0;
		 pidProfiles[i].simplified_pids_mode = PID_SIMPLIFIED_TUNING_RPY;
		 pidProfiles[i].simplified_master_multiplier = SIMPLIFIED_TUNING_DEFAULT;
		 pidProfiles[i].simplified_roll_pitch_ratio = SIMPLIFIED_TUNING_DEFAULT;
		 pidProfiles[i].simplified_i_gain = SIMPLIFIED_TUNING_DEFAULT;
		 pidProfiles[i].simplified_d_gain = SIMPLIFIED_TUNING_D_DEFAULT;
		 pidProfiles[i].simplified_pi_gain = SIMPLIFIED_TUNING_DEFAULT;
		 pidProfiles[i].simplified_dmin_ratio = SIMPLIFIED_TUNING_D_DEFAULT;
		 pidProfiles[i].simplified_feedforward_gain = SIMPLIFIED_TUNING_DEFAULT;
		 pidProfiles[i].simplified_pitch_pi_gain = SIMPLIFIED_TUNING_DEFAULT;
		 pidProfiles[i].simplified_dterm_filter = true;
		 pidProfiles[i].simplified_dterm_filter_multiplier = SIMPLIFIED_TUNING_DEFAULT;
	 }
